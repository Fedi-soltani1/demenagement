...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const clientEmail = dossier.clientId
...
function buildEmailHtml(dossier: { numeroDossier?: string; nomComplet?: string; prixTotalTTC?: number; devisValiditeJours?: number }): string {
  const validite = dossier.devisValiditeJours ?? 30
  const prix = dossier.prixTotalTTC != null
    ? `${dossier.prixTotalTTC.toLocaleString('fr-TN')} DT TTC`
    : 'sur demande'
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;">
        <tr>
          <td style="background:#b52027;padding:24px 32px;">
            <h1 style="color:#ffffff;margin:0;font-size:22px;font-weight:bold;">DT Déménagement Tunisie</h1>
            <p style="color:#c9a84c;margin:4px 0 0;font-size:12px;letter-spacing:1px;">VOTRE DEVIS EN PIÈCE JOINTE</p>
          </td>
        </tr>
        <tr>
          <td style="padding:32px;">
            <p style="color:#333;font-size:15px;">Bonjour ${dossier.nomComplet ?? 'cher client'},</p>
            <p style="color:#555;font-size:14px;line-height:1.7;">
              Nous avons le plaisir de vous faire parvenir votre devis <strong>${dossier.numeroDossier ?? ''}</strong>
              en pièce jointe à cet email.
            </p>
            <div style="background:#fafafa;border:1px solid #e0e0e0;border-radius:6px;padding:16px;margin:20px 0;">
              <p style="margin:0 0 8px;font-size:13px;color:#999;text-transform:uppercase;letter-spacing:1px;">Montant total TTC</p>
            ...
            <p style="color:#555;font-size:14px;line-height:1.7;">
              ...
              Pour l'accepter ou pour toute question, contactez-nous :
            </p>
        ...
        <tr>
          <td style="background:#1a1a1a;padding:16px 32px;text-align:center;">
            <p style="color:#888;font-size:11px;margin:0;">
              DT Déménagement Tunisie — demenagement.tn<br />
              +216 52 880 311 | contact@demenagement.tn
            </p>
          </td>
        </tr>
      </table>
