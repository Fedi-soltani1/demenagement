...
interface VilleLivePreviewWrapperProps {
  initialVille:      VilleDoc
  locale:            string
  slug:              string
  telephone?:        string
...
export function VilleLivePreviewWrapper({
  initialVille,
  telephone,
  settings,
  services,
  ...
  pays,
  googleReviewsNode,
}: VilleLivePreviewWrapperProps) {
  const { data } = useLivePreview<VilleDoc>({
    initialData: initialVille,
