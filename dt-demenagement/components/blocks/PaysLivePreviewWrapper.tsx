...
interface PaysLivePreviewWrapperProps {
  initialPays:       PaysDoc
  locale:            string
  slug:              string
  telephone?:        string
...
export function PaysLivePreviewWrapper({
  initialPays,
  telephone,
  settings,
  services,
  ...
  pays,
  googleReviewsNode,
}: PaysLivePreviewWrapperProps) {
  const { data } = useLivePreview<PaysDoc>({
    initialData: initialPays,
