...
// Autorise `limit` requêtes puis bloque, par clé.
{
  ...
  for (let i = 0; i < 3; i++) assert.equal(rateLimit(key, 3, 60_000), true, `req ${i + 1} autorisée`)
  assert.equal(rateLimit(key, 3, 60_000), false, '4e req bloquée (limite 3)')
}
...
// Les clés sont indépendantes.
{
  ...
  assert.equal(rateLimit(a, 1, 60_000), true, 'A 1re ok')
  assert.equal(rateLimit(a, 1, 60_000), false, 'A 2e bloquée')
  assert.equal(rateLimit(b, 1, 60_000), true, 'B indépendante de A')
}

// La fenêtre expirée réautorise.
{
  const key = `test-d-${Math.random()}`
  assert.equal(rateLimit(key, 1, 1), true, '1re ok (fenêtre 1ms)')
  const start = Date.now()
  while (Date.now() <= start + 2) { /* attendre > 1ms */ }
  assert.equal(rateLimit(key, 1, 1), true, 'réautorisée après expiration fenêtre')
}
